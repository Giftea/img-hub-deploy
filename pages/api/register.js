...
export default async function register(req, res) {
  ...
  const userExist = await xata.db.users.filter({ email }).getFirst(); // check if user exists

  if (userExist) {
    res.status(400);
    throw new Error("User already exists"); // throw error if user with email exists
  }
